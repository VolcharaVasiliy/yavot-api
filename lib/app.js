...
export async function app({ method = "GET", pathname = "/", headers = {}, body = null }) {
  ...
  // YANDEX_SESSION_ID может храниться в base64 (vercel env режет спецсимволы).
  // Декодируем, если строка похожа на base64 и раскодируется в сессию.
  let rawSession = norm(sessionId || process.env.YANDEX_SESSION_ID);
  if (rawSession) {
    try {
      const decoded = base64.decode(rawSession);
      // раскодируем только если результат похож на сессию Яндекса (нач. "3:" или содержит ":")
      if (typeof decoded === "string" && (decoded.startsWith("3:") || decoded.includes(":"))) {
        rawSession = decoded;
      }
    } catch {}
  }
  ...
  try {
    ...
    if (lively) {
      ...
      if (!Object.keys(h).length) {
        // Нет авторизации на сервере — сразу понятная ошибка вместо 500.
        return {
          status: 400,
          headers: { ...CORS, "Content-Type": "application/json" },
          body: {
            error: "Живая озвучка недоступна: на сервере yavot-api не задан YANDEX_SESSION_ID",
            detail: "Добавьте переменную окружения YANDEX_SESSION_ID (можно в base64) в настройках Vercel, либо переводите обычной озвучкой (lively=false).",
            hint: "Без авторизации Яндекс не отдаёт живой голос; обычный перевод работает без неё.",
          },
        };
      }
      // Отправляем сессию/токен в Яндекс при живой озвучке.
